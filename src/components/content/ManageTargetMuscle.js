...
import { assignExerciseTargetMuscl, popExerciseTargetMuscl } from '../helper/api';
...
const ManageTargetMuscle = (props) => {
    ...
    const [Gym,setGym]=useState({totalMembers:0,totalBranch:0})
    ...
    const getThisGym=()=>{
        getGym(user._id,token,ItemId).then(data=>{
          if (data.error){
            ...
            setTargetMuscleList(data.tgmsclList);
          }
        })
    }


    // Hooks call to Open Create, & Delete Target Muscles Dialog Form
    const [open, setOpen] = useState(false);

    const handleCreateOpen = () => {
        setSwitchForm({
            formName: "Target Muscle",
            formType: 0,
        });
        setOpen(true);
    }
    ...
    const {targetMscl} = TargetMuscle;

    const onCreate = event => {
        event.preventDefault();

        // Create Request to Backend 
        assignExerciseTargetMuscl(user._id,token,ItemId,{targetMscl}).then(data => {
            if(data.error){
    ...
    const onDelete =item=>event=> {
        ...
        popExerciseTargetMuscl(user._id,token,ItemId,{item}).then(data=>{
            if(data.error){
              console.log('error in db')
            }else{
                handleClose();
    ...
    const handleChange = name => event => {
        setTargetMuscle({...TargetMuscle,[name]:event.target.value});
    }
    ...
    return(
        <Dashboard flag={flag} itemId={ItemId} data={{totalMembers,totalBranch,state:"none"}}>
            <div className="row" style={{justifyContent: "space-between"}}>
                ...
                <p className="mr-5 pt-4" style={{fontSize: 16}}>Target Muscle</p>
            </div>

            <div className="pt-3 pl-3 pr-3">
            {
              TargetMuscleList.map((data)=>{
                return(
            ...
            <Dialog open={open} onClose={handleClose} TransitionComponent={Transition}>
                {
                    (formType === 0)?(
                        <div className="pr-4 pl-2">
                            ...
                            <div>
                                <input onChange={handleChange("targetMscl")} value={targetMscl} className="pl-4 pr-4 pb-3 pt-3 mb-3 ml-4 shadow" type="text" id="gymName" name="gymName" autoFocus="true" placeholder="Name" style={{outline: "none", borderRadius: 30, width: "90%", border: "none", color: "rgb(255, 81, 0)"}}/>
                            </div>
                        </div>
                    )
                    : (formType === 1)?(
                        ...
                        <div className="ml-5 mt-4 mr-5 mb-4">
                           {/* Delete Exercise Type */}
                            ...
                            Do you really want to delete this <br/> Target Muscle?
...
export default ManageTargetMuscle;